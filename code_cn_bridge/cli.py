...
@click.group()
@click.version_option(version=__version__, prog_name="lan-bridge")
def main():
...
@click.option(
    ...
    help="配置文件路径（默认: ~/.lan-bridge.yaml）",
)
@click.option(
    "-p", "--port",
    default=None,
...
def start(config: str | None, port: int | None, host: str | None, verbose: bool):
    ...
    # 设置配置路径
    if config:
        os.environ["LAN_BRIDGE_CONFIG"] = config

    # 导入配置（触发加载）
    from .config import get_config
    cfg = get_config(config)

    bind_host = cfg.server_host if host is None else host
    bind_port = cfg.server_port if port is None else port
    try:
        cfg.set_runtime_server_address(bind_host, bind_port)
    except (TypeError, ValueError) as exc:
        raise click.ClickException(str(exc)) from exc

    click.echo(f"LAN BRIDGE v{__version__}")
...
@click.option(
    ...
    default=None,
    type=click.Path(),
    help="配置文件输出路径（默认: ~/.lan-bridge.yaml）",
)
def init(provider: str, api_key: str | None, output: str | None):
    """初始化配置文件"""
    from .config import Config

    output_path = Path(output) if output else Path.home() / ".lan-bridge.yaml"
    ...
    click.echo("  2. 启动代理: lan-bridge start")
...
def validate(config_path: str):
    ...
    try:
        ...
        click.echo(f"[OK] 配置文件有效: {config_path}")
